...
// Read the .jq template from a real file (not a template literal) so the
// jq source can use its native escapes (`\(...)`) without TS string
// escaping, gets IDE syntax highlighting, and stays editable as a normal
// `.jq` file. The template ships in the npm tarball via package.json's
// `files: ["dist", "templates"]`.
const generateNotifySlackJq = (): string => {
	const here = path.dirname(fileURLToPath(import.meta.url));
	// dist/commands/init.js → repo root is two levels up
