...
The message format is defined in `.kagemusha/notify-slack.jq` (generated by `kagemusha init`). Edit that file to customize. Test locally:

```bash
jq -r -f .kagemusha/notify-slack.jq reports/summary.json
```

The same `summary.json` works for Discord (swap `text` → `content`) or PR comments via `actions/github-script` — copy the jq file and tweak. Slack auto-unfurls the URLs into image previews when your bucket is public-read.
